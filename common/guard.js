...
<script type="module">
  ...
  async function mustBe(minRole = 'usuario') {
    ...
    window.currentUser = { id:s.user.id, email:s.user.email, role:me.role, nombre:me.nombre ?? null };
    return true;
  }

  async function requireAuth(){ return mustBe('usuario'); }
  async function signOutAndRedirect(){ await window.db.auth.signOut(); location.replace(LOGIN_PAGE); }
